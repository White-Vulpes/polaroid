...
import { Fullscreen, Share, Slideshow, Zoom } from "yet-another-react-lightbox/plugins";
...
function Like() {
    ...
    const handleClick = useCallback(() => {
        const url = currentSlide.src.split('/');
        handleLiked(`${url[0]}//${url[2]}/THUMBNAIL/${url[3]}_THUMBNAIL_${url[4]}`);
        handleLiked(currentSlide.src);
    },[currentSlide, handleLiked]);
    return (
        <div className="con-like">
        <input className="like" type="checkbox" checked={getLiked(currentSlide.src)} onChange={handleClick} title="like"/>
...
function LightBox(props) {
    ...
    return <div className="album-slideshow" onClick={() => setIndex(0)}>
              ...
              <Lightbox
                ...
                plugins={[Fullscreen, Slideshow, Zoom, Share]}
                toolbar={{buttons: [<Like />,'zoom','slideshow', 'fullscreen', 'close' ]}}
              />
            </div>
}
